...
/*   Updated: 2018/01/09 07:53:14 by elbenkri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_print_flags_buffer4(t_var *var, t_flags *s_flags, int res)
{
	...
	{
		var->res = ((var->type == TYPE_STRING || var->type == TYPE_WSTRING) && !(s_flags->c & (1 << 0))) ? ft_strcpy(tab, "(null)") : 0;
		ft_flags_precision(var, s_flags, 0);
	}
	if (var->type == TYPE_WCHAR || (var->type == TYPE_CHAR && s_flags->m > 2 && s_flags->m <= 16))
	{
